...
open class TimeIntervalPicker: UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    private let fontBold: UIFont = .boldSystemFont(ofSize: 15)
    ...
    private func initialize() {
        let screenWidth: CGFloat = UIScreen.main.bounds.width
        let screenHeight: CGFloat = UIScreen.main.bounds.height
        let width: CGFloat = 300 //(screenWidth - 64)
        let height: CGFloat = 280 //250
        let x: CGFloat = (screenWidth - width) / 2 //32
    ...
    private func updateUI() {
        ...
        self.closeButton?.titleLabel?.font = self.fontBold
        self.doneButton?.titleLabel?.font = self.fontBold
        self.titleLabel?.font = self.boldFont
        
        self.closeButton?.setTitleColor(UIColor.blue, for: .normal)
    ...
    open func show(at selectedMinute: Int = 0) {
        ...
        //let bgView = UIView()
        bgContainerView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: UIScreen.main.bounds.size.height)
        bgContainerView.backgroundColor = UIColor.black.withAlphaComponent(0.5)
        window?.addSubview(bgContainerView)
